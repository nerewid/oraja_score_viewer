...
import { logger } from './utils/logger.js'; // デバッグログ用ロガーをインポート
...
document.getElementById("processData").addEventListener("click", async () => {
    ...
    try {
        logger.time("prepation"); // 処理時間の計測を開始（準備）
        ...
        logger.timeEnd("prepation"); // 処理時間の計測終了（準備）
        ...
        logger.time("find scores"); // 処理時間の計測を開始（スコア検索）
        ...
        logger.timeEnd("find scores"); // 処理時間の計測終了（スコア検索）

        logger.time("create json"); // 処理時間の計測を開始（JSON作成）
        ...
        logger.timeEnd("create json"); // 処理時間の計測終了（JSON作成）
...
function createSongDataMap(songs) { // md5をキーとしたマップを作成
    logger.time("create songdatamap"); // 処理時間の計測を開始（楽曲データMap作成）
    ...
    logger.timeEnd("create songdatamap"); // 処理時間の計測終了（楽曲データMap作成）
...
async function createSha256ToMd5Map(db, songs) {
    logger.time("append songdatamap"); // 処理時間の計測を開始（楽曲データMap追加）
    ...
    logger.timeEnd("append songdatamap"); // 処理時間の計測終了（楽曲データMap追加）
